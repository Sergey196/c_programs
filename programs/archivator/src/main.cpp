...
int ReadBit( BFILE *bfile )
{
   ...
   if ( bfile->mask == 0x80 )
   {
      ...
      if ( bfile->rack == EOF )
         fatal_error((char *)"Error in function ReadBit!\n");
      if ( ( bfile->pacifier_counter++ & PACIFIER_COUNT ) == 0 )
         putc( '.', stdout );
   }
...
unsigned long ReadBits ( BFILE *bfile, int bit_count )
{
  ...
  bfile->rack = getc( bfile->file );
  if ( bfile->rack == EOF )
            fatal_error((char *)"Error in function ReadBits!\n");
  if ( ( bfile->pacifier_counter++ & PACIFIER_COUNT ) == 0 )
            putc( '.', stdout );
      }
...
int main(int argc, char * argv[])
{
   ...
   if(argv[1][0] == 'e' || argv[1][0] == 'E')
   {
      ...
      if (input == NULL) {
         fatal_error((char *)"Ошибка при открытии %s для ввода\n", argv[ 2 ]);
      }
   
      // открытие выходного файла для записи
      output = OpenOutputBFile( argv[ 3 ] );
      if(output == NULL) {
         fatal_error((char *)"Ошибка при открытии %s для вывода\n", argv[ 3 ]);
      }
      printf( "\nКомпрессия %s в %s\n", argv[ 2 ], argv[ 3 ] );
   ...
   else if(argv[1][0] == 'd' || argv[1][0] == 'D')
   {
      ...
      if(input == NULL) {
         fatal_error((char *)"Error on open %s for read\n", argv[2]);
      }
    
      // открытие выходного файла для записи
      output = fopen(argv[3], "wb" );
      if(output == NULL) {
         fatal_error((char *)"Error on open %s for write\n", argv[3]);
      }
